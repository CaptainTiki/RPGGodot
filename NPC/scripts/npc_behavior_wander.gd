...
func _set_wander_range(v : int ) -> void:
	wander_range = v
	$CollisionShape2D.shape.radius = v * 32.0 #32 is how many pixles per tile in the tilemap
...
func _ready():
	if Engine.is_editor_hint():
		return
	super() #runs parent ready - then come back to run the rest
